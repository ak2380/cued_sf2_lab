...
def jpegenc_dwt(X, qstep, rise, n = 3, M = 8, opthuff = False, dcbits = 8, log = True):
    '''
    ...
    if log:
        print('Performing {}-level DWT'.format(n))
    Y = nlevdwt(X, n)
    # fig, ax = plt.subplots()
    # plot_image(Y, ax=ax);
    ...
    ssrs = compute_ss_ratios(n)
    dwtstep = ssrs * qstep
    Yq, _ = dwtquant(Y, dwtstep, rise)

    # fig, ax = plt.subplots()
    # plot_image(Yq, ax=ax);

    if log:
        print('Regrouping into {} x {} blocks'.format(N,N))
    Yq = dwtgroup(Yq, n)
...
def jpegdec_dwt(vlc, qstep, rise, n = 3, M = 8, hufftab = None, dcbits = 8, W = 256, H = 256, log = True):
    ...
    ssrs = compute_ss_ratios(n)
    dwtstep = ssrs * qstep
    Zq = dwtquant_reconstruct(Z_reg, dwtstep, rise)
...
def objective_function_dwt(step, image, rise, target_bits, n=3, M=8, opthuff=True):
    """
    ...
    Parameters:
        ...
        n (int): Number of levels for the DWT.
        opthuff (bool): Whether to use optimized Huffman coding.

    Returns:
        float: The absolute difference between the actual bit count and target bit count.
    """
    step = max(0, step)  # Ensure the step size is non-negative
    vlc, hufftab, totalbits = jpegenc_dwt(image, step, rise, n=n, M=M, opthuff=opthuff, log=False)
